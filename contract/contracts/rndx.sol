...
contract Ownable {
    ...
    modifier onlyOwnerOrOperator() { require(msg.sender == owner || operators[msg.sender] == msg.sender); _; }
    ...
    function addOperator(address _operator) external onlyOwner {
        ...
        require(operators[_operator] == address(0));
        emit OperatorAdded(_operator);
        operators[_operator] = _newOperator;
    }

    function removeOperator(address _operator) external onlyOwner {
        require(operators[_operator] == msg.sender || (operators[_operator] != 0 && msg.sender == owner));
...
contract BlackList is Ownable {
    ...
    function addManyToBlacklist(address[] _lockAddressArray) external onlyOwnerOrOperator {
        for (uint256 i = 0; i < _lockAddressArray.length; i++) {
          blackList[_lockAddressArray[i]] = true;
        }
    }
...
// @Project Round X
// ----------------------------------------------------------------------------
contract RNDX is PausableToken, MintableToken, BurnableToken, MultiTransferToken {
    string public name = "RoundX";
    string public symbol = "RNDX";
